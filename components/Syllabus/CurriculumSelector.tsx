...
export default function CurriculumSelector() {
    ...
    const [curriculumList, setCurriculumList] = useState(['-- Select a curriculum --']);
    ...
    useEffect(() => {
        if (!loaded.current) {
            loaded.current = true;
            const getData = async () => {
                const data = await fetch('/api/getCurriculumList').then(d => d.json());
    ...
    return <>
        <label>
            ...
            >
                {curriculumList.map((c, i) => <option disabled={!i ? true : false} key={c}>{c}</option>)}
            </select>
            <br />
            <br />
        </label>
